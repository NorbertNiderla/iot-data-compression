#include "../../../../compression_iot/archive/source/linux_7_dim/sprintz.h"
...
#include "../../../../compression_iot/archive/source/linux_7_dim/fire.h"
...
/*
	if(size == 8){
		for(int i = 0; i<DIM*size; i+=DIM) printf("%d\n", int_buffer[i]); //DEBUG
		for(int i = 1; i<DIM*size; i+=DIM) printf("%d\n", int_buffer[i]); //DEBUG
		//printf("\n");
	}
*/

	//liczenie bitów
		for(int i = 0;i<DIM*size;i++) fire_bits += (int)(32 - __builtin_clz(abs(int_buffer[i]))+1);
